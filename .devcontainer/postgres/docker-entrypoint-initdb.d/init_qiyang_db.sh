...
if [ "$WORKSPACE_POSTGRES_INIT" == 'true' ]; then
	psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
		CREATE USER $WORKSPACE_POSTGRES_USER WITH PASSWORD '$WORKSPACE_POSTGRES_PASSWD';
		CREATE DATABASE $WORKSPACE_POSTGRES_DB;
		GRANT ALL PRIVILEGES ON DATABASE $WORKSPACE_POSTGRES_DB TO $WORKSPACE_POSTGRES_USER;
		ALTER ROLE $WORKSPACE_POSTGRES_USER CREATEROLE SUPERUSER;
	EOSQL
	echo
fi
