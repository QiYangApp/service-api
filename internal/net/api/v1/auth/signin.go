...
func SignInPost(ctx *gin.Context, form *auth.SignInForm) *resp.Response {
	...
	if err != nil {
		if errors.Is(err, errs.ErrNotExist) || errors.Is(err, errs.ErrInvalidArgument) {
			...
			return resp.Fail(ctx, err.Error(), http.StatusBadRequest, "FORM.USERNAME_PASSWORD_INCORRECT")
		} else if usertype.IsErrUserNotExist(err) {
			log.Sugar().Infof("Failed authentication attempt for %s from %s: %v", form.UserName, ctx.RemoteIP(), err)

			return resp.Fail(ctx, err.Error(), http.StatusBadRequest, "FORM.ACCOUNT_BEEN_USED")
			...
			return resp.Error(ctx, err, http.StatusInternalServerError, nil)
		}
	}
	...
	if skipper, ok := source.Cfg.Value().(authserver.LocalTwoFASkipper); ok && skipper.IsSkipLocalTwoFA() {
		if userSession, err := authserver.SignInUserSession(ctx, u, form.Remember); err != nil {
			...
			return resp.Success(ctx, userSession)
		}
	}
	...
	if err != nil {
		...
		return resp.Error(ctx, err, http.StatusInternalServerError, "")
	}
