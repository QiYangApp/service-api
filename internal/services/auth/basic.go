...
import (
	...
	"net/http"
	ent "service-api/internal/ent"
	"service-api/internal/modules/base"
	"service-api/internal/util"
	"strings"
)
...
func (b *Basic) Verify(req *http.Request, w http.ResponseWriter, store DataStore, sess SessionStore) (*ent.User, error) {
	// Basic authentication should only fire on API, Download or on Git or LFSPaths
	if !util.IsAPIPath(req) && !util.IsContainerPath(req) && !util.IsAttachmentDownload(req) {
		return nil, nil
	}
