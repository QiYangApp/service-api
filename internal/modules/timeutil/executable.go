...
func GetExecutableModTime() time.Time {
	executablModTimeOnce.Do(func() {
		...
		if err != nil {
			log.Client().Sugar().Error("os.Executable: %v", err)
			return
		}

		exePath, err = filepath.Abs(exePath)
		if err != nil {
			log.Client().Sugar().Error("filepath.Abs: %v", err)
			return
		}

		exePath, err = filepath.EvalSymlinks(exePath)
		if err != nil {
			log.Client().Sugar().Error("filepath.EvalSymlinks: %v", err)
			return
		}

		st, err := os.Stat(exePath)
		if err != nil {
			log.Client().Sugar().Error("os.Stat: %v", err)
			return
		}
