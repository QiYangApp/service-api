...
func (*WakatimeDependency) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case wakatimedependency.FieldID, wakatimedependency.FieldWakatimeID, wakatimedependency.FieldUserID, wakatimedependency.FieldTotalSeconds, wakatimedependency.FieldCreateTime, wakatimedependency.FieldUpdateTime:
	...
	for i := range columns {
		...
		case wakatimedependency.FieldCreateTime:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				wd.CreateTime = timeutil.TimeStamp(value.Int64)
			}
		case wakatimedependency.FieldUpdateTime:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field update_time", values[i])
			} else if value.Valid {
				wd.UpdateTime = timeutil.TimeStamp(value.Int64)
			}
		default:
...
func (wd *WakatimeDependency) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", wd.CreateTime))
	builder.WriteString(", ")
	builder.WriteString("update_time=")
	builder.WriteString(fmt.Sprintf("%v", wd.UpdateTime))
	builder.WriteByte(')')
