...
func (*UserRelatedRole) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case userrelatedrole.FieldID, userrelatedrole.FieldUserID, userrelatedrole.FieldCreateTime, userrelatedrole.FieldUpdateTime:
	...
	for i := range columns {
		...
		case userrelatedrole.FieldCreateTime:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				urr.CreateTime = timeutil.TimeStamp(value.Int64)
			}
		case userrelatedrole.FieldUpdateTime:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field update_time", values[i])
			} else if value.Valid {
				urr.UpdateTime = timeutil.TimeStamp(value.Int64)
			}
		default:
...
func (urr *UserRelatedRole) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", urr.CreateTime))
	builder.WriteString(", ")
	builder.WriteString("update_time=")
	builder.WriteString(fmt.Sprintf("%v", urr.UpdateTime))
	builder.WriteByte(')')
