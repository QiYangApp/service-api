...
func (*MemberRoleRelatedPermission) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case memberrolerelatedpermission.FieldID, memberrolerelatedpermission.FieldCreateTime, memberrolerelatedpermission.FieldUpdateTime:
	...
	for i := range columns {
		...
		case memberrolerelatedpermission.FieldCreateTime:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				mrrp.CreateTime = timeutil.TimeStamp(value.Int64)
			}
		case memberrolerelatedpermission.FieldUpdateTime:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field update_time", values[i])
			} else if value.Valid {
				mrrp.UpdateTime = timeutil.TimeStamp(value.Int64)
			}
		default:
...
func (mrrp *MemberRoleRelatedPermission) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", mrrp.CreateTime))
	builder.WriteString(", ")
	builder.WriteString("update_time=")
	builder.WriteString(fmt.Sprintf("%v", mrrp.UpdateTime))
	builder.WriteByte(')')
