...
func (c *Manage) init() *Manage {
	...
	if err := c.SetDefaultDrive(c.drive); err != nil {
		log.Sugar().Fatalf("register default cache drive fail: %v", err)
	}
...
func (c *Manage) SetDefaultDrive(key string) error {
	...
	if err != nil {
		log.Sugar().Warnf("set default cache dirver fail, err: %v", err)
		return err
	}
...
func (c *Manage) GetDefaultDrive() Drive {
	var err error
	if c.drive == "" {
		log.Sugar().Fatal("drive not found")
	}
	...
	if err != nil {
		log.Sugar().Fatalf("register cache drive fail: %v", err)
	}
...
func (c *Manage) Register(key string) (Drive, error) {
	...
	conn := conf.Client().GetStringMap("conns." + key)
	if len(conn) == 0 {
		log.Sugar().Panic("cache database config conns empty")
	}
