...
func GenerateApi(scan *Scan, storageDir string) {
	...
	newFile.ImportName("framework/router", "router")
	...
	newFile.Func().Id("init").Params().Block(
		jen.Qual("framework/router", "Register").Call(registerCode...),
		jen.Qual("framework/router", "Apis").Op("=").Map(jen.String()).Op("*").
			Qual("framework/router", "MethodInfo").
